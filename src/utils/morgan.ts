...
const developmentEnvironment = (app: Express) => {
  return app.use(morgan(":ip - :method :url :status :response-time ms - :res[content-length]"));
};

const productionEnvironment = (app: Express) => {
  return app.use(morgan(':ip - :remote-user [:date[clf]] ":method :url HTTP/:http-version" :status :res[content-length] ":referrer" ":user-agent"'));
};
