...
const apiRateLimiter = (req: Request, res: Response, next: NextFunction) => {
    rateLimiter.consume(req.clientIp || "")
        .then((rateLimiterRes) => {
            res.set({
        ...
        .catch((rateLimiterRes) => {
            ...
            next(new HttpException("TooManyRequestError", StatusCode.TooManyRequests.status, "Too many request. Try again later",
                StatusCode.TooManyRequests.name, req.startTime))
        });
}
